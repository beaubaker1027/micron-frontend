...
class Search extends Component {
  ...
  fetchData = async (text, max = 15) => {
    await fetch(`${process.env.REACT_APP_FETCH_ALL_API_ENDPOINT}query=${text}&max${max}`)
  ...
  renderListItems = data => {
    if(data.errors) return <span key={0} className='informer'>{('No results to display').toUpperCase()}</span>;
    let {list} = data;
    let {item} = list;
    if(!list || !item) return;
  ...
  render() {
    return (
      <div style={{height:'100vh'}}>
        ...
        <div className='viewPort' style={{overflow:'auto', boxSizing:'border-box', minHeight:'90vh', maxHeight:'90vh'}}>
        <div style={{textAlign:'center'}}>
          <input
            ...
            onBlur={()=>{this.setState({placeholder: 'Food Search'})}}
            onChange={() => {
              this.setState({
            ...
            <ul className='contentList'>
            {
              !!this.state.data ?
                ...
                this.state.search ?
                  ...
                  :
                  <span className='informer'>Search Must Be Atleast 3 Characters</span>
            }
            </ul>
        </div>)
        :
